...
func (r *ReagentTypeRepo) Create(ctx context.Context, dto *models.ReagentTypeDTO) (string, error) {
	query := fmt.Sprintf(`INSERT INTO %s (id, name, role_id) VALUES (:id, :name, :role_id)`, ReagentTypesTable)
	id := uuid.New()
	dto.Id = id.String()
...
func (r *ReagentTypeRepo) Update(ctx context.Context, dto *models.ReagentTypeDTO) error {
	query := fmt.Sprintf(`UPDATE %s SET name=:name, role_id=:role_id WHERE id=:id`, ReagentTypesTable)
