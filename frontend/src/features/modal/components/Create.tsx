	...
	const saveControlHandler = async (data: IControlForm, isShouldUpdate?: boolean) => {
		...
		if (!closing && !isForceSave.current) {
			const newData: ICreateDataItem = {
				typeId: base.type,
				name: base.name.trim(),
				uname: base.uname.trim(),
				document: base.document.trim(),
				purity: base.purity.trim(),
				dateOfManufacture: base.dateOfManufacture,
				consignment: base.consignment.trim(),
				manufacturer: base.manufacturer.trim(),
				shelfLife: +base.shelfLife,
				place_closet: base.place_closet.trim(),
				place_shelf: +base.place_shelf,
				receiptDate: data.receiptDate,
				amount: +data.amount,
				amountTypeId: data.amountType,
				controlDate: data.controlDate,
				protocol: data.protocol.trim(),
				result: data.result,
			}
